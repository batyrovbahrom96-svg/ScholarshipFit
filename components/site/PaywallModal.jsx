...
const TRUST_BADGES = [
  ...
  { icon: Trophy,  text: '800 hand-verified scholarships', sub: 'Source-linked · no dead links · no aggregator spam' },
]
...
export default function PaywallModal({ open, onClose, matchCount = 0, totalWorth = 0, initialPlan = 'annual' }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      // Server-side tracking for abandoned-checkout recovery (fire-and-forget)
      fetch('/api/paywall/track', {
        method: 'POST',
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="mt-6 flex flex-col items-center justify-center gap-3 border-t border-white/5 pt-6 text-center">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-xs text-white/40">
              ...
              <span>✓ 800 hand-verified scholarships</span>
            </div>
          </div>
        </div>
      </DialogContent>
