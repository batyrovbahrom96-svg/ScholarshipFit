      ...
      <div className="relative">
        ...
        <div className="container mx-auto max-w-4xl px-4 py-10 relative">
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm text-slate-400">{STEPS[i].title}</p>
          </div>
          <Progress value={progress} className="mt-3 h-1.5 bg-white/5 [&>div]:bg-cyan-400"/>

          <Card className="mt-6 glass-strong border-white/10">
            <CardContent className="p-6 md:p-8">
              ...
              {step==='achievements' && (
                <StepShell title="Achievements & experience" caption="Free-text — awards, olympiads, leadership, research, volunteering, work, portfolio.">
                  <textarea value={form.achievements} onChange={e=>upd('achievements', e.target.value)} rows={6} placeholder="e.g. 2x national robotics finalist, 1 year research internship in fluid dynamics, volunteer STEM mentor..." className="w-full rounded-lg border border-white/10 bg-white/[0.04] p-3 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-400/40"/>
                </StepShell>
              )}
              ...
              {step==='preferences' && (
                <StepShell title="Preferences" caption="Countries, intake, budget, and funding requirements.">
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.04] p-3"><span className="text-sm text-slate-200">Full funding only</span><Switch checked={form.full_funding_only} onCheckedChange={v=>upd('full_funding_only',v)}/></div>
                    <div className="flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.04] p-3"><span className="text-sm text-slate-200">Partial funding acceptable</span><Switch checked={form.partial_funding_ok} onCheckedChange={v=>upd('partial_funding_ok',v)}/></div>
                  </div>
                </StepShell>
              )}
                ...
                <StepShell title="Documents you already have" caption="Check what's ready. Missing items will appear on your dashboard checklist.">
                  <div className="grid gap-3 sm:grid-cols-2">
                    {[
                      ...
                      <label key={k} className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/[0.04] p-3 cursor-pointer hover:border-cyan-400/30">
              ...
              {step==='processing' && (
                <div className="text-center py-6">
                  <div className="relative mx-auto h-24 w-24">
                    <div className="absolute inset-0 rounded-full border-2 border-cyan-400/30" style={{animation:'orbit 6s linear infinite'}}/>
                    <div className="absolute inset-3 rounded-full border border-cyan-400/50" style={{animation:'orbit 4s linear infinite reverse'}}/>
                  ...
                  <p className="mt-1 text-sm text-slate-400">Claude Sonnet 4.5 is analyzing your profile against every source-linked record.</p>
                  <ul className="mt-6 mx-auto max-w-md space-y-2 text-left">
                    {PROC_LINES.map((l, idx) => (
                      <li key={idx} className={`flex items-center gap-3 rounded-lg border px-3 py-2 text-sm ${idx <= procIdx ? 'border-cyan-400/30 bg-cyan-500/10 text-cyan-100' : 'border-white/10 bg-white/[0.03] text-slate-500'}`}>
              ...
              {step!=='processing' && (
                <div className="mt-6 flex items-center justify-between">
                  <Button variant="ghost" onClick={back} disabled={i===0} className="text-slate-300 hover:text-white hover:bg-white/5"><ArrowLeft className="mr-2 h-4 w-4"/>Back</Button>
                  {i < STEPS.length - 2 ? (
                    <Button onClick={next} className="bg-cyan-500 text-black hover:bg-cyan-400">Continue <ArrowRight className="ml-2 h-4 w-4"/></Button>
                  ) : (
                    <Button onClick={next} className="bg-orange-500 hover:bg-orange-400 text-white"><Rocket className="mr-2 h-4 w-4"/>Run AI Match</Button>
                  )}
                </div>
              )}
            </CardContent>
          ...
          <p className="mt-4 text-center text-xs text-slate-500">ScholarshipFit provides informational scholarship research only. It does not guarantee admission, scholarships, visas, or funding.</p>
        </div>
      </div>
      <Footer />
    </div>
...
function Field({ label, val, on, placeholder, type='text', className='' }) {
  return (
    <div className={className}>
      ...
      <Input type={type} placeholder={placeholder} value={val} onChange={e=>on(e.target.value)} className="mt-1 bg-white/[0.04] border-white/10 text-slate-100 placeholder:text-slate-500"/>
    </div>
  )
}
