  ...
  return (
    <div className="cosmos-bg min-h-screen">
      ...
      <div className="container mx-auto max-w-4xl px-4 py-14">
        ...
        <p className="mt-2 text-slate-400">Transparent scoring, source-linked records, honest limits.</p>
        ...
        <Section title="How matching works" icon={<Brain className="h-4 w-4"/>}>
          ...
          <ul className="mt-3 grid gap-1 text-slate-300 [&>li]:list-disc [&>li]:ml-6">
        ...
        <Section title="What the AI can do" icon={<Check className="h-4 w-4"/>}>
          <ul className="grid gap-1 text-slate-300 [&>li]:list-disc [&>li]:ml-6">
        ...
        <p className="mt-8 text-xs text-slate-500">Individual results vary. ScholarshipFit does not guarantee similar outcomes.</p>
      </div>
      <Footer />
    </div>
  )
  ...
  return (
    <Card className="mt-6 border-white/10 bg-white/[0.03]">
      <CardContent className="p-6">
        ...
        <div className="mt-3 space-y-2 text-slate-300 leading-relaxed">{children}</div>
      </CardContent>
    </Card>
  )
}
