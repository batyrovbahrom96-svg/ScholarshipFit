      ...
      <div className="container mx-auto max-w-7xl px-4 py-10">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-slate-400 text-sm">Manage database records, trust levels, and AI activity.</p>
          </div>
          <Button onClick={()=>setCreating(true)} className="bg-cyan-500 text-black hover:bg-cyan-400"><Plus className="mr-2 h-4 w-4"/>Add scholarship</Button>
        </div>
                ...
                <tbody>
                  {items.map(s => (
                    <tr key={s.id} className="border-b border-white/5 text-slate-200">
                      <td className="p-3"><div className="font-medium text-white">{s.scholarship_name}</div><div className="text-xs text-slate-400">{s.university_name}</div></td>
                      ...
                      <td className="p-3">{s.public_status === 'hidden' ? <Badge className="bg-amber-500/20 text-amber-200 hover:bg-amber-500/20">hidden</Badge> : <Badge className="bg-emerald-500/20 text-emerald-200 hover:bg-emerald-500/20">public</Badge>}</td>
                      <td className="p-3 text-right">
                        <Button size="sm" variant="ghost" onClick={()=>setEditing(s)} className="text-slate-300 hover:text-white hover:bg-white/5"><Pencil className="h-4 w-4"/></Button>
                        <Button size="sm" variant="ghost" onClick={()=>togglePublic(s)} className="text-slate-300 hover:text-white hover:bg-white/5">{s.public_status === 'hidden' ? <Eye className="h-4 w-4"/> : <EyeOff className="h-4 w-4"/>}</Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <Card className="border-white/10 bg-white/[0.03]"><CardContent className="p-5">
            <p className="text-[11px] uppercase tracking-widest text-slate-400">Recent AI match runs</p>
            <div className="mt-3 space-y-2 max-h-80 overflow-y-auto pr-1">
              {logs?.match_runs?.length ? logs.match_runs.map(r=>(
                <div key={r.id} className="rounded-md border border-white/10 bg-white/[0.02] p-3 text-sm">
                  <div className="flex items-center justify-between text-xs text-slate-400"><span>{new Date(r.created_at).toLocaleString()}</span><span>{r.result?.matches?.length || 0} matches</span></div>
                  <p className="mt-1 text-slate-300 line-clamp-2">{r.result?.summary}</p>
                </div>
              )) : <p className="text-sm text-slate-500">No runs yet.</p>}
            </div>
          </CardContent></Card>
          <Card className="border-white/10 bg-white/[0.03]"><CardContent className="p-5">
            <p className="text-[11px] uppercase tracking-widest text-slate-400">Recent advisor messages</p>
            <div className="mt-3 space-y-2 max-h-80 overflow-y-auto pr-1">
              {logs?.advisor_messages?.length ? logs.advisor_messages.map(m=>(
                <div key={m.id} className="rounded-md border border-white/10 bg-white/[0.02] p-3 text-sm">
                  <div className="flex items-center justify-between text-xs text-slate-400"><span>{m.role}</span><span>{new Date(m.created_at).toLocaleTimeString()}</span></div>
                  <p className="mt-1 text-slate-300 line-clamp-3">{m.content}</p>
                </div>
              )) : <p className="text-sm text-slate-500">No advisor traffic yet.</p>}
            </div>
          </CardContent></Card>
        </div>
      </div>
...
function Stat({ icon, label, value }) {
  return (
    <Card className="border-white/10 bg-white/[0.03]"><CardContent className="p-5">
      <div className="flex items-center gap-2 text-cyan-300">{icon}<span className="text-xs uppercase tracking-widest text-slate-400">{label}</span></div>
...
function ScholarshipDialog({ open, initial, isNew, onClose, onSave }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl bg-[#0a0f1c] border-white/10 text-slate-100">
        ...
        <div className="grid gap-3 sm:grid-cols-2 max-h-[70vh] overflow-y-auto pr-2">
          ...
          <div>
            <label className="text-[11px] uppercase tracking-widest text-slate-400">Trust level</label>
            <Select value={doc.trust_level} onValueChange={v=>upd('trust_level',v)}>
              <SelectTrigger className="mt-1 bg-white/[0.04] border-white/10 text-slate-100"><SelectValue/></SelectTrigger>
              <SelectContent>
          ...
          <div>
            <label className="text-[11px] uppercase tracking-widest text-slate-400">Public status</label>
        ...
        <DialogFooter>
          <Button variant="ghost" onClick={onClose} className="text-slate-300 hover:text-white hover:bg-white/5">Cancel</Button>
          <Button onClick={()=>onSave(doc, isNew)} className="bg-cyan-500 text-black hover:bg-cyan-400">Save</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
...
function F({ label, v, on, className='' }) {
  return (
    <div className={className}>
      ...
      <Input value={v||''} onChange={e=>on(e.target.value)} className="mt-1 bg-white/[0.04] border-white/10 text-slate-100"/>
    </div>
  )
}
