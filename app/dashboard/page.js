      ...
      <div className="container mx-auto max-w-7xl px-4 py-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">Your AI shortlist, powered by Claude Sonnet 4.5 — source-linked, honest, no invented results.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Button onClick={rerun} disabled={rematching} className="bg-cyan-500 text-black hover:bg-cyan-400">
              <RefreshCw className={`mr-2 h-4 w-4 ${rematching?'animate-spin':''}`}/>{rematching?'Rematching...':'Rerun AI match'}
            </Button>
            <Link href="/onboarding"><Button variant="outline" className="border-white/10 bg-transparent text-slate-100 hover:bg-white/5">Edit profile</Button></Link>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-12">
              ...
              <CardContent className="p-3">
                <nav className="flex flex-col gap-1">
                  {SIDEBAR.map(item => item.href ? (
                    <Link key={item.key} href={item.href} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white">
                      <span className="text-cyan-300">{item.icon}</span>{item.label}
                    </Link>
                  ) : (
                    <button key={item.key} onClick={()=>setTab(item.key)}
                      className={`flex items-center gap-2 rounded-md px-3 py-2 text-sm text-left ${tab===item.key ? 'bg-cyan-500/10 text-white border border-cyan-400/25' : 'text-slate-300 hover:bg-white/5 hover:text-white border border-transparent'}`}>
                      <span className={tab===item.key ? 'text-cyan-300' : 'text-slate-400'}>{item.icon}</span>{item.label}
                    </button>
                  ))}
                </nav>
              </CardContent>
          ...
          <div className="lg:col-span-9 space-y-6">
              ...
              <Card className="border-cyan-400/30 bg-cyan-500/5">
                <CardContent className="p-6 flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-slate-400">Complete onboarding to see your first AI shortlist.</p>
                  </div>
                  <Link href="/onboarding"><Button className="bg-orange-500 hover:bg-orange-400 text-white"><Rocket className="mr-2 h-4 w-4"/>Start onboarding</Button></Link>
                </CardContent>
              </Card>
            )}
            ...
            {/* AI Summary */}
            {(summary || advisory) && (
              <Card className="border-white/10 bg-gradient-to-br from-cyan-500/10 to-white/[0.02]">
                <CardContent className="p-5">
                  <p className="text-[11px] uppercase tracking-widest text-cyan-300">AI portfolio summary</p>
                  {summary && <p className="mt-1 text-slate-100">{summary}</p>}
                  {advisory && <p className="mt-2 text-sm text-slate-300 italic">{advisory}</p>}
                  <p className="mt-3 flex items-center gap-1.5 text-[11px] text-slate-500"><Info className="h-3 w-3"/> ScholarshipFit provides informational scholarship research only. Users apply directly through official provider websites.</p>
                </CardContent>
              </Card>
            )}
              ...
              <Card className="border-white/10 bg-white/[0.03]">
                <CardContent className="p-5">
                  <div className="flex items-center justify-between">
                    <p className="text-[11px] uppercase tracking-widest text-slate-400">Document checklist — across your top matches</p>
                    <Badge variant="outline" className="border-white/10 bg-white/5 text-slate-200">{requiredDocs.length} items</Badge>
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {requiredDocs.map((d,i)=>(<Badge key={i} variant="outline" className="border-amber-500/30 bg-amber-500/10 text-amber-200">{d}</Badge>))}
                  </div>
                </CardContent>
              </Card>
            )}
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                {matches.length > 0 && <span className="text-sm text-slate-500">{filtered.length} of {matches.length}</span>}
              </div>
              <div className="mt-4 grid gap-4">
                {filtered.length === 0 && (
                  <Card className="border-white/10 bg-white/[0.02]"><CardContent className="p-10 text-center text-slate-400">
                    <p>Nothing here yet.</p>
                    <p className="mt-1 text-xs text-slate-500">Try another tab or rerun the AI match.</p>
                  </CardContent></Card>
                )}
                {filtered.map((m,i)=>(
